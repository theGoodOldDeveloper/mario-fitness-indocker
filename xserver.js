...
app.post("/createuser/", bodyParser.json(), async (req, res) => {
  ...
  var member = req.body;
  sql =
    "INSERT INTO users (username, emil, password,isinmember) VALUES (?,?,?,?)";
  await db.run(sql, [member.username, member.emil, "alma", true], (err) => {
    if (err) return console.error(err);
  });
  //res.end()
  res.header("Access-Control-Allow-Origin", "*");
...
app.post("/createtrainingbooking/", bodyParser.json(), async (req, res) => {
  ...
  sql =
    "INSERT INTO trainings (trainingID, name, trainingDATE, personID, bookingDATE) VALUES (?,?,?,?,?)";
  await db.run(
    sql,
    [
      member.trainingID,
...
cron.schedule("* * * * 1", () => {
  // INFO - at 2023-04-24 00:00:00 (now 04-18) all Monday
  getdata();
});
